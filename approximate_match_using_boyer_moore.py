...
class ApproximateMatching:
    """
    ApproximateMatching: This class provides a method to find
    approximate matches of a pattern in a DNA sequence with
    a specified number of allowed mismatches.

    It divides the pattern into sub-patterns and uses
    the Boyer-Moore algorithm to search for approximate
    matches while considering the maximum number of
    allowed mismatches.
    """
    ...
    def approximate_match(self, data, pattern, num_mismatch):
        """
        Find approximate matches of a pattern in a DNA sequence with a maximum number of allowed mismatches.

        Parameters:
        - data (str): The DNA sequence to search in.
        - pattern (str): The pattern to search for.
        - num_mismatch (int): The maximum number of allowed mismatches.

        Returns:
        - list: List of starting indices of approximate matches.
        """
        hits = set()  # A set to store all non-redundant matches
        len_subpattern = int(len(pattern) / (num_mismatch + 1))  # Length of each subpattern

        for i in range(num_mismatch + 1):
            start = i * len_subpattern
            end = min((i + 1) * len_subpattern, len(pattern))

            p_bm = BoyerMoorePreProcessing(pattern[start:end])  # Create an object of pre-processing for each subpattern
            model = BoyerMooreAlgorithm()  # Create an object of the Boyer-Moore algorithm for each subpattern
            occurrences = model.boyer_moore(pattern[start:end], p_bm, data)  # Call the Boyer-Moore function

            for occurrence in occurrences:
                if occurrence < start or occurrence - start + len(pattern) > len(data):
                    continue

                mismatches = 0
                for j in range(0, start):
                    if not pattern[j] == data[occurrence - start + j]:
                        mismatches += 1
...
# Create an instance of the ApproximateMatching class
model = ApproximateMatching()

# Define the maximum number of allowed mismatches
num_mismatch = 2

# Find approximate matches of the pattern in the DNA sequence
hits = model.approximate_match(data, pattern, num_mismatch)